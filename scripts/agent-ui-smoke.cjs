...
function response(name, args) {
  if (name === 'damecon_get_equipment' && !Object.keys(args).length) {
  ...
  return { schemaVersion: '1.0', revision: 1, capturedAt: '2026-09-09T00:00:00.000Z', source: { status: 'live' }, data: { mock: true, tool: name, args }, warnings: [] }
}
...
async function run() {
  ...
  for (const name of [
    ...
    assert(JSON.stringify(call.args) === '{}', `${name} did not omit optional arguments`)
  }
  ...
  assert(JSON.stringify(blankEnumCall.args) === '{}', 'blank optional enum was sent')
  ...
  assert(JSON.stringify(equipmentBlank.args) === '{}', 'blank equipment options were not omitted')
